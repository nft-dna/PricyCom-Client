...
import { FACTORY_ERC721_ABI } from './abi';
...
export const useFactoryContract = () => {
  ...
  const getERC721FactoryContract = async () =>
    await getContract(Contracts[CHAIN].erc721Factory, FACTORY_ERC721_ABI);

  //const getPrivateFactoryContract = async () =>
  //  await getContract(Contracts[CHAIN].privateFactory, FACTORY_ERC721_ABI);

  const getERC1155FactoryContract = async () =>
    await getContract(Contracts[CHAIN].erc1155Factory, FACTORY_ERC721_ABI);

  //const getPrivateArtFactoryContract = async () =>
  //  await getContract(Contracts[CHAIN].privateArtFactory, FACTORY_ERC721_ABI);
